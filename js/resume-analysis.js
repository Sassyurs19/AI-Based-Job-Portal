// Consolidated Resume Analysis page script
...
document.addEventListener('DOMContentLoaded', function() {
  ...
  // Check for pre-existing resume
  const user = api.getCurrentUser();
  if (user && user.resume) {
    showUploadedState(user.resume);
    if (jobSelect) {
      jobSelect.disabled = false;
      if (jobSelect.options[0]) {
        jobSelect.options[0].textContent = 'Select a job to analyze against';
      }
    }
  }
});

function showUploadedState(resumePath) {
  ...
  uploadZone.innerHTML = `
    <div class="file-info" style="display:flex; flex-direction:column; align-items:center; gap:12px; padding:24px;">
      ...
      <div>
        <h4 style="margin:0; font-size:1.1rem;">Current Resume: ${fileName}</h4>
        <p style="margin:4px 0 0 0; color:var(--muted); font-size:0.9rem;">Select a job below to start analysis, or upload a different file.</p>
      </div>
      <button class="btn btn-sm btn-ghost" id="changeFileBtn" style="margin-top:8px;">Upload Different File</button>
    </div>
  `;
...
function setupFileUpload() {
  ...
  const chooseBtn = uploadZone ? uploadZone.querySelector('.btn-grad') : null;
...
async function handleFileSelect(file) {
    ...
    uploadZone.innerHTML = `
      <div class="file-info" style="display:flex; flex-direction:column; align-items:center; gap:12px; padding:24px;">
        ...
        <div>
          ...
          <p style="margin:4px 0 0 0; color:var(--muted); font-size:0.9rem;">Uploading...</p>
        </div>
      </div>
    `;
  }
  ...
  try {
    ...
    if (result.success) {
      ...
      showUploadedState(result.resume);
...
function resetUpload() {
  ...
  if (jobSelect) {
    ...
    jobSelect.value = '';
    if (jobSelect.options[0]) {
      jobSelect.options[0].textContent = 'Upload your resume first';
    }
  }
  const analyzeBtn = document.getElementById('analyzeBtn');
  if (analyzeBtn) {
...
function displayAnalysisResults(analysis) {
  ...
  analysisResults.innerHTML = `
    <div class="resume-main" style="width: 100%;">
      ...
      <div class="scores-grid" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(220px, 1fr)); gap: 20px; margin-bottom: 32px;">
        <!-- Overall Match Ring -->
        <div class="score-card" style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 24px;">
          <div class="progress-ring" style="position: relative; width: 100px; height: 100px;">
            <svg width="100" height="100" style="transform: rotate(-90deg);">
              <circle class="progress-ring-bg" cx="50" cy="50" r="42" fill="none" stroke="rgba(255,255,255,0.06)" stroke-width="6"/>
              <circle class="progress-ring-fill" cx="50" cy="50" r="42" fill="none" stroke="var(--grad)" stroke-width="6" stroke-linecap="round" stroke-dasharray="263.89" stroke-dashoffset="263.89"/>
            </svg>
            <div class="progress-ring-text" style="position: absolute; inset: 0; display: grid; place-items: center; font-family: 'Space Grotesk'; font-size: 1.3rem; font-weight: 700; color: white;">${analysis.overallScore || 0}%</div>
          </div>
          <div class="label" style="margin-top: 12px; font-size: 0.85rem; color: var(--muted); font-weight: 500;">Overall Match</div>
        </div>
      ...
      <div class="sidebar-card" style="margin-top: 24px;">
        <h3>Resume Tips</h3>
        <div class="analysis-list" style="display: grid; grid-template-columns: repeat(auto-fit, minmax(280px, 1fr)); gap: 16px;">
          <div class="analysis-item" style="padding:16px;">
            ...
            <div class="analysis-item-content">
              <h4 style="font-size:.9rem;">Keep it concise</h4>
              <p style="font-size:.8rem;">Aim for 1-2 pages maximum to engage busy recruiters.</p>
            </div>
          </div>
          <div class="analysis-item" style="padding:16px;">
            <div class="analysis-item-icon blue">
            ...
            <div class="analysis-item-content">
              <h4 style="font-size:.9rem;">Use action verbs</h4>
              <p style="font-size:.8rem;">Start your bullet points with strong, impactful action verbs.</p>
            </div>
          </div>
        </div>
      </div>
  ...
  if (ring) {
    ...
    const offset = ((100 - score) / 100) * 263.89;
    setTimeout(() => {
  ...
  if (improveBtn) {
    improveBtn.addEventListener('click', () => {
      showSuccess('Redirecting to smart resume builder...');
    });
  }
  ...
  try {
    ...
    if (result.success && result.jobs) {
      if (result.jobs.length === 0) {
        container.innerHTML = '<p style="color:var(--muted)">No matching jobs found.</p>';
        return;
      }
