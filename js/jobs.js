...
document.addEventListener('DOMContentLoaded', function() {
  ...
  document.querySelectorAll('.filter-chip').forEach(chip => {
    chip.addEventListener('click', function() {
      ...
      handleSearch(); // trigger search filter update
    });
  });
...
function renderJobs(jobs) {
  ...
  if (jobs.length === 0) {
    jobsContainer.innerHTML = '<div class="no-jobs" style="color: var(--muted); text-align: center; padding: 40px; grid-column: 1/-1;">No jobs found matching the filters</div>';
    return;
  }
    ...
    <article class="job-card gborder">
      ...
      <div class="job-footer">
        ...
        <div class="job-actions">
          <button class="btn btn-sm btn-ghost" onclick="saveJobFromList('${job._id}')">Save</button>
...
function renderJobDetails(job) {
  ...
  if (saveBtn) {
    saveBtn.addEventListener('click', () => saveJobFromList(job._id));
  }
}
...
async function saveJobFromList(jobId) {
  try {
...
// Map old global functions to prevent crashes on list items
window.saveJobFromList = saveJobFromList;
